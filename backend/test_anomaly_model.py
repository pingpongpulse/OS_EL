#!/usr/bin/env python3
"""
Test script to validate if anomaly_model (1).pkl is valid and working.
"""
...
def test_anomaly_model():
    """Test the anomaly_model (1).pkl file."""
    print("Testing anomaly_model (1).pkl...")
    
    # Define the path to the model file
    model_path = os.path.join(os.path.dirname(__file__), 'models', 'anomaly_model (1).pkl')
    ...
    try:
        # Sample feature data (5 features based on the trained model)
...
def test_with_anomaly_detector_class():
    """Test using the AnomalyDetector class by modifying it to load the correct model file."""
    ...
    # Create a modified detector that points to the correct model file
    model_path = os.path.join(os.path.dirname(__file__), 'models', 'anomaly_model (1).pkl')
    
    try:
        # Create detector with the specific model path
...
if __name__ == "__main__":
    print("🚀 Starting anomaly model validation test...\n")
    
    success = test_anomaly_model()

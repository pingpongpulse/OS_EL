...
def test_anomaly_model_standard():
    ...
    # Test the model with 7 features (expected by current AnomalyDetector)
    print("\n🧪 Testing model with 7-feature data...")
    try:
        ...
        print(f"❌ Error during 7-feature model testing: {e}")
        ...
        try:
            ...
            try:
                sample_data_3 = np.array([
                    [10.0, 30.0, 2.0],  # Normal usage
                    [98.0, 20.0, 1.5],  # High CPU (potential anomaly)
                    [15.0, 95.0, 8.0]   # High memory (potential anomaly)
                ])
                
                print(f"📋 Sample data shape (3 features): {sample_data_3.shape}")
                
                # Test prediction
                predictions = model.predict(sample_data_3)
                print(f"🎯 Model predictions (3 features): {predictions}")
                print(f"📊 Number of anomalies detected: {sum(predictions == -1)}")
                
                # Test if model has decision_function or score_samples
                if hasattr(model, 'decision_function'):
                    scores = model.decision_function(sample_data_3)
                    print(f"📈 Decision function scores: {scores}")
                elif hasattr(model, 'score_samples'):
                    scores = model.score_samples(sample_data_3)
                    print(f"📈 Score samples: {scores}")
                
                print("✅ Model test with 3 features completed successfully!")
                return True
                
            except Exception as e3:
                print(f"❌ Error during 3-feature model testing: {e3}")
                return False
